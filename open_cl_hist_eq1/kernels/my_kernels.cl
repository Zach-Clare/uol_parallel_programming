...
kernel void hist_simple(global const int* A, global int* H, int nr_bins, int padding, int min_value, int max_value) { 
	int id = get_global_id(0);

	//assumes that H has been initialised to 0
	int bin_index = A[id];//take value as a bin index

	if (bin_index == padding) { // if recieved value is a padding element
		return;
	}
